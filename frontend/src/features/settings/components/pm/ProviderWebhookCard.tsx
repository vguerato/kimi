...
import { useGetNgrokInfo } from '../../jira/api/useGetNgrokInfo';
import { useSetupProviderWebhook } from '../../jira/api/useSetupWebhook';

interface ProviderWebhookCardProps {
    provider: string;
}

export function ProviderWebhookCard({ provider }: ProviderWebhookCardProps) {
    const { data: ngrokInfo = { url: null, webhookUrl: null } } = useGetNgrokInfo();
    const { mutate: setupWebhook, isPending } = useSetupProviderWebhook(provider);
        ...
        <div className="rounded-xl p-6 flex flex-col gap-5 bg-surface border border-border">
            <div className="flex items-center justify-between">
                ...
                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold
                    ${isActive ? 'bg-success-soft text-success' : 'bg-default text-muted'}`}>
